...
def options_chains(ticker):
    ...
    print(f"Latest close price for SHV ETF (1-3 month ETF): {latest_r_one_to_three_months}")

    # 1-3 years ETF
    ticker = yf.Ticker("SHY")
    info = ticker.info
    latest_r_one_to_three_years = info.get("dividendYield")
    print(f"Latest close price for SHV ETF (1-3 year ETF): {latest_r_one_to_three_years}")

    # Now that we have approximations for the risk-Free rate, we can begin to 
    THREE_MONTH_T = 0.2548 # this is approximately 3*31 / 365  (i.e 3 months in days / 365)
    chains["r"] = np.where(chains["T"] > THREE_MONTH_T, latest_r_one_to_three_years, latest_r_one_to_three_months)  #conditionally set the risk free rate for the data
    return chains


# Now we will begin by creating the function which is supposed to calculate the implied volatility.
def calculate_implied_volatility(n_steps: int, C_mkt: float, T: float, S0: float, K: float, r: float, hist_vol: float):
...
def calc_hist_vol(ticker):
    aggs = []
    for a in client.list_aggs(ticker=ticker, multiplier=1, timespan="day", from_= datetime.datetime.now().date() - datetime.timedelta(days=30), to=datetime.datetime.now().date(), limit=50000):
